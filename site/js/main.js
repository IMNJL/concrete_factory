...
// Main app script (extracted from index2.html)
document.addEventListener('DOMContentLoaded', ()=>{
  // --- Data model ---
  const DEFAULT_DATA = {
    prices: [
  ...
  function loadData(){
    try{
      const raw = localStorage.getItem('bz_data')
  ...
  function renderMarkSelect(){
    ...
    appData.prices.forEach(p=>{
      ...
      opt.value = p.mark
      opt.textContent = `${p.mark} — ${formatPrice(p.price)} ₽/м³`
      sel.appendChild(opt)
    })
  }
  function renderPriceTable(){
  ...
  if(calcBtn){
    calcBtn.addEventListener('click',()=>{
      const mark = document.getElementById('markSelect').value
      const vol = parseFloat(document.getElementById('volume').value)||0
      const km = parseFloat(document.getElementById('deliveryKm').value)||0
      const priceObj = appData.prices.find(p=>p.mark===mark)
      if(!priceObj){document.getElementById('calcResult').textContent='Выберите марку';return}
      const base = priceObj.price * vol
      // Простая модель доставки: базовая ставка 1500 + 50 ₽/км
      const delivery = 1500 + (50 * km)
      const total = Math.round(base + delivery)
      document.getElementById('calcResult').textContent = `Итог: ${formatPrice(total)} ₽ (бетон ${formatPrice(Math.round(priceObj.price*vol))} ₽ + доставка ${formatPrice(delivery)} ₽)`
    })
  }
  if(clearBtn){
    clearBtn.addEventListener('click',()=>{document.getElementById('volume').value='1';document.getElementById('deliveryKm').value='5';document.getElementById('calcResult').textContent='—'})
  }
  ...
  if(authBtn){
    authBtn.addEventListener('click',()=>{
      const pass = document.getElementById('adminPass').value
      // Пароль хранится в JS — при необходимости замените на серверную авторизацию
      if(pass==='admin123'){
        document.getElementById('adminArea').style.display='block'
        document.getElementById('adminPass').value=''
        renderAdminTable()
